...
import lume.ai.data.remote.protocol.SUBPROTOCOLO
import lume.ai.data.remote.protocol.lerMensagem
    ...
    fun conectar(
        ...
        val ouvinte = object : WebSocketListener() {
            ...
            override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
                trySend(EventoDoCanal.Encerrado(null, null, reason.ifBlank { null }))
                close()
            }

            override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                trySend(EventoDoCanal.Encerrado(t, response?.code, t.message))
                close()
            }
        }
