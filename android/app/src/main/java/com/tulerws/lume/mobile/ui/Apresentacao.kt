...
import com.tulerws.lume.mobile.ui.theme.StateColor
...
/**
 * Texto de uma falha de ação, a partir do código do protocolo.
 *
 * `permission_gone` **não** aparece aqui: o protocolo o define como situação
 * normal, e o ViewModel o converte no cartão "respondida em outro dispositivo".
 */
@StringRes
fun rotuloDaFalha(codigo: String): Int = when (codigo) {
    "desconectado", "timeout" -> R.string.acao_falhou_desconectado
    "session_busy" -> R.string.acao_falhou_ocupada
    "action_not_available", "session_not_found" -> R.string.acao_falhou_indisponivel
    else -> R.string.acao_falhou_outro
}
