<script lang="ts">
  ...
  let dragState: { pointerId: number; grabX: number; grabY: number } | null = null;
  ...
  function beginDrag(event: PointerEvent) {
    ...
    dragState = {
      ...
      grabX: event.clientX * windowState.scale,
      grabY: event.clientY * windowState.scale,
    };
  }

  function moveDrag(event: PointerEvent) {
    if (!dragState || dragState.pointerId !== event.pointerId || !windowState) return;
    const dx = event.clientX * windowState.scale - dragState.grabX;
    const dy = event.clientY * windowState.scale - dragState.grabY;
    if (!dragMoved && Math.hypot(dx, dy) < 2) return;
    event.preventDefault();
    dragMoved = true;
    queueMove(windowState.x + dx, windowState.y + dy);
  }
  ...
  function moveResize(event: PointerEvent) {
    if (!resizeDragState || resizeDragState.pointerId !== event.pointerId || !windowState) return;
    event.preventDefault();
    event.stopPropagation();
    const scale = windowState.scale;
    const dx = event.clientX * scale - resizeDragState.grabX;
    const dy = event.clientY * scale - resizeDragState.grabY;
    const east = resizeDragState.direction.endsWith("East");
    const south = resizeDragState.direction.startsWith("South");
    const desiredWidth = windowState.width + (east ? dx : -dx) / scale;
    const desiredHeight = windowState.height + (south ? dy : -dy) / scale;
    ...
    pendingResize = {
      x: east ? windowState.x : windowState.x + Math.round((windowState.width - width) * scale),
      y: south ? windowState.y : windowState.y + Math.round((windowState.height - height) * scale),
      width,
      height,
    };
    void flushResize();
...
<style>
  ...
  .terminal-card { position: relative; width: 100%; height: 100%; display: flex; flex-direction: column; overflow: hidden; border: 1px solid rgba(103, 126, 116, 0.2); border-radius: 17px; color: #26342e; background: rgb(248, 251, 249); box-shadow: inset 0 -4px 14px rgba(20, 36, 29, 0.055); transition: border-color 150ms ease, box-shadow 180ms ease, background-color 180ms ease; }
  .terminal-card > header { min-height: 48px; padding: 7px 8px 7px 9px; display: flex; align-items: center; gap: 7px; border-bottom: 1px solid rgba(97, 119, 109, 0.11); cursor: grab; touch-action: none; user-select: none; }
  .terminal-card.dragging > header { cursor: grabbing; }
  .terminal-card.resizing { user-select: none; }
  .terminal-card.dock-moving { border-color: rgba(72, 142, 111, 0.5); box-shadow: inset 0 0 0 2px rgba(75, 157, 120, 0.12), inset 0 -4px 14px rgba(31, 91, 66, 0.08); }
  .terminal-card.dock-target { border-color: rgba(65, 151, 111, 0.7); box-shadow: inset 0 0 0 3px rgba(75, 157, 120, 0.15); }
  .terminal-card.settling { border-color: rgba(69, 139, 108, 0.42); box-shadow: inset 0 0 0 2px rgba(75, 157, 120, 0.1); }
  ...
  .terminal-window.dark .terminal-card { color: #dbe7e1; border-color: rgba(190, 209, 200, 0.13); background: rgb(20, 29, 25); }
  .terminal-window.dark .terminal-card.dock-moving,
  .terminal-window.dark .terminal-card.dock-target,
  .terminal-window.dark .terminal-card.settling { border-color: rgba(91, 186, 143, 0.5); box-shadow: inset 0 0 0 2px rgba(91, 186, 143, 0.1); }
  ...
  .terminal-window.dark header button:hover { color: #cfdcd5; background: rgba(205, 222, 213, 0.09); }
  .terminal-window.dark .terminate-confirm > span { color: #d8b8b2; }
  .terminal-window.dark .terminate-confirm button { color: #c9d6cf; border-color: rgba(205, 222, 213, 0.14); background: rgba(218, 232, 225, 0.055); }
  .terminal-window.dark .terminate-confirm button.danger { color: #e09a9a; border-color: rgba(224, 140, 140, 0.25); }
  .terminal-window.dark .message { color: #e09a9a; }
  .terminal-window.dark .status-failed,
  .terminal-window.dark .status-failed span { color: #e08c8c; }
  .terminal-window.dark .status-running,
  .terminal-window.dark .status-running span { color: #8db8e0; }
  .terminal-window.dark .status-permission_required,
  .terminal-window.dark .status-permission_required span,
  .terminal-window.dark .status-waiting_for_input,
  .terminal-window.dark .status-waiting_for_input span { color: #d9b06a; }
